        ...
        <div id="taskContainer" class="flex flex-col gap-4">
            @forelse($tasks as $task)
                ...
                <a href="{{ route('student.task.show', $task->id) }}"
                    ...
                    data-title="{{ strtolower($task->title) }}" data-deadline="{{ $task->deadline ?? '9999-12-31' }}">
                    ...
                    <div class="flex-1 p-5 space-y-2">
                        ...
                        <div class="flex flex-wrap gap-2">
                            ...
                            @if (!$submission)
                                <span
                                    class="px-2 py-0.5 rounded-full text-xs font-semibold bg-gray-100 text-gray-700 dark:bg-gray-700 dark:text-gray-300">
                                    Belum Dikerjakan
                                </span>
                            @else
                                @switch($statusName)
                                    @case('pending')
                                        <span
                                            class="px-2 py-0.5 rounded-full text-xs font-semibold bg-yellow-100 text-yellow-700 dark:bg-yellow-900 dark:text-yellow-300">Sudah
                                            Dikumpulkan</span>
                                    @break

                                    @case('in progress')
                                        <span
                                            class="px-2 py-0.5 rounded-full text-xs font-semibold bg-blue-100 text-blue-700 dark:bg-blue-900 dark:text-blue-300">Sedang
                                            Dinilai</span>
                                    @break

                                    @case('completed')
                                        <span
                                            class="px-2 py-0.5 rounded-full text-xs font-semibold bg-green-100 text-green-700 dark:bg-green-900 dark:text-green-300">Selesai</span>
                                    @break

                                    @default
                                        <span
                                            class="px-2 py-0.5 rounded-full text-xs font-semibold bg-gray-100 text-gray-700 dark:bg-gray-700 dark:text-gray-300">Tidak
                                            Diketahui</span>
                                @endswitch
                            @endif
                        </div>
                    ...
                    <div
                        class="bg-gray-50 dark:bg-gray-900 w-full md:w-32 flex items-center justify-center md:flex-col md:justify-between p-4">
                        <span class="text-xs text-gray-500 dark:text-gray-400 md:mb-2">Lihat Detail</span>
                        <span
                            class="text-blue-600 dark:text-blue-400 text-2xl font-semibold group-hover:translate-x-1 transition-transform">→</span>
                    </div>
                </a>
        ...
        <script>
            document.addEventListener('DOMContentLoaded', () => {
                ...
                function initializeCards() {
                    ...
                    allCards = Array.from(cardElements).map(card => ({
                        element: card.cloneNode(true),
                        title: (card.dataset.title || card.querySelector('h3')?.textContent || '')
                            .toLowerCase().trim(),
                        deadline: card.dataset.deadline || '9999-12-31'
                    }));
                }
                ...
                function showEmptyState(isSearch = false) {
                    if (defaultEmpty) {
                        defaultEmpty.style.display = (allCards.length === 0 && !isSearch) ? 'block' : 'none';
                    }
                    if (searchEmpty) {
                        searchEmpty.style.display = isSearch ? 'block' : 'none';
                    }
                }
                ...
                function filterAndSort() {
                    ...
                    let filteredCards = allCards.filter(cardData => {
                        ...
                        return cardData.title.includes(keyword);
                    });
                    ...
                    if (filteredCards.length > 0) {
                        filteredCards.forEach(cardData => {
                            const card = cardData.element.cloneNode(true);

                            card.dataset.title = cardData.title;
                            card.dataset.deadline = cardData.deadline;
                        ...
                        showEmptyState(keyword.length > 0);
                    }
                }
